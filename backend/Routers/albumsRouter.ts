...
albumsRouter.get("", async (req, res) => {
  try {
    if (req.query.artist) {
      ...
      const result: IAlbum[] = await Album.find({ artist: queryId }).sort(
        "-releaseYear",
      );
...
albumsRouter.patch(
  ...
  async (req, res) => {
    try {
      ...
      albumById.save();
      return res.send({
        message: `Album switched to ${
          albumById.isPublished ? "published mode" : "not published mode"
        }`,
      });
    } catch {
...
albumsRouter.delete("/:id", auth, permit("admin"), async (req, res) => {
  try {
    ...
    if (tracksByAlbum.length) {
      return res.send({
        message: "The album should be empty before deleting it",
      });
    }
